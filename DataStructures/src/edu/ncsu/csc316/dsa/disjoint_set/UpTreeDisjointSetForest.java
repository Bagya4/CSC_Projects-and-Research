...
public class UpTreeDisjointSetForest<E> implements DisjointSetForest<E> {
	...
	/** map structure for disjoint sets */
    ...
    public Position<E> makeSet(E value) {
    	UpTreeNode<E> e = new UpTreeNode<E>(value);
    	map.put(value, e);
    	return e;
    ...
    public Position<E> find(E value) {
    	...
    	UpTreeNode<E> curr = map.get(value);
    	//implementing path compression
    	return findHelper(curr);
    	
    }

    private UpTreeNode<E> findHelper(UpTreeNode<E> current) {
    	
    	UpTreeNode<E> c = current;
    	if(c.getParent() != c) {
    		c.setParent(findHelper(c.getParent()));
    	}
    	
    	return c.getParent();
        // Implementing path-compression find
    }
