...
public class HeapAdaptablePriorityQueue<K extends Comparable<K>, V> extends HeapPriorityQueue<K, V>
    ...
    public void remove(Entry<K, V> entry) {
    	AdaptablePQEntry<K, V> l = validate(entry);
    	int j = l.getIndex();
    	if(j == list.size() - 1) {
    		...
    		swap(j, list.size() - 1); //swap entry to last pos
    		list.remove(list.size() - 1); //removing pos
    		bubble(j); //fixing entry displaced by swap
    	}
    }
