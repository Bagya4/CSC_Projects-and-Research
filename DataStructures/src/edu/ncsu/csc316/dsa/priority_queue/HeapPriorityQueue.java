...
public class HeapPriorityQueue<K extends Comparable<K>, V> extends AbstractPriorityQueue<K, V> {
    ...
    protected void upHeap(int index) {
    	...
    	while(j > 0) {
    		int p = parent(j);
    		if(compare(list.get(j).getKey(), list.get(p).getKey()) >= 0) break;
    		swap(j, p);
    		j = p;
    	}
    ...
    protected void downHeap(int index) {
    	...
    	while(hasLeft(j)) {
    		...
    		if(hasRight(j)) {
    			...
    			if(compare(list.get(left).getKey(), list.get(rightIdx).getKey()) > 0) {
    				smallChild = rightIdx;
    			}
    		}
    		
    		if(compare(list.get(smallChild).getKey(), list.get(j).getKey()) >= 0) break;
