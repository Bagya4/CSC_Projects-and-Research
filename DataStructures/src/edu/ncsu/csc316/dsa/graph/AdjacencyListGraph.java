...
public class AdjacencyListGraph<V, E> extends AbstractGraph<V, E> {
    ...
    public Edge<E> insertEdge(Vertex<V> v1, Vertex<V> v2, E edgeData) {
        ...
        if(isDirected()) {
        	...
        	Position<Edge<E>> oin = origin.getOutgoing().addLast(edge);
        	edge.setOutgoingListPosition(oin);
        	
        	Position<Edge<E>> dout = destination.getIncoming().addLast(edge);
            edge.setIncomingListPosition(dout);
            
        }
        
        else { //undirected graph
        	Position<Edge<E>> originout = origin.getOutgoing().addLast(edge);
        	edge.setOutgoingListPosition(originout);
        	
//        	Position<Edge<E>> originin = origin.getIncoming().addLast(edge);
//        	edge.setIncomingListPosition(originin);
//        	
//        	destination.getIncoming().addLast(edge);
//            destination.getOutgoing().addLast(edge);
        ...
        return edge;
        // Setting the edge's positions in the outgoingEdges
    ...
    public void removeVertex(Vertex<V> vertex) {
        ...
        for(Edge<E> e: v.getOutgoing()) {
        	removeEdge(e);
        	if(v.getOutgoing().size() == 0) break;
        }
        for(Edge<E> e: v.getIncoming()) {
        	removeEdge(e);
        	if(v.getIncoming().size() == 0) break;
        }
