...
public class ShortestPathUtil {
    ...
    public static <V, E extends Weighted> Map<Vertex<V>, Integer> dijkstra(Graph<V, E> graph, Vertex<V> start) {
    	...
    	//map to track shortest path to each vertex
    	Map<Vertex<V>, Integer> weights = new LinearProbingHashMap<>();
    	//adaptable PQ to order vertices by weight
    	AdaptablePriorityQueue<Integer, Vertex<V>> vWeight = new HeapAdaptablePriorityQueue<>();
        ...
        for(Vertex<V> v: graph.vertices()) {
            ...
            pqEntries.put(v, vWeight.insert(weights.get(v), v));
        }
        
        while(!vWeight.isEmpty()) {
        	Vertex<V> u = vWeight.deleteMin().getValue();
        	...
        	for(Edge<E> e: graph.outgoingEdges(u)) {
        		...
        		int r = weights.get(u) + e.getElement().getWeight();
        		if(!known.contains(z) && r < weights.get(z)) {
        			weights.put(z, r);
        			vWeight.replaceKey(pqEntries.get(z), r);
        		}
        	}
        }
    ...
    public static <V, E extends Weighted> Map<Vertex<V>, Edge<E>> shortestPathTree(Graph<V, E> graph, Vertex<V> start, Map<Vertex<V>, Integer> costs) {
    	// map to store edges in the shortest path tree
    	Map<Vertex<V>, Edge<E>> edges = new LinearProbingHashMap<Vertex<V>, Edge<E>>();
