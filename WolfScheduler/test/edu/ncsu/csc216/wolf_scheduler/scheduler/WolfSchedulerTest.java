...
public class WolfSchedulerTest {
	...
	public void testGetCourseFromCatalog() {
		...
		Product c = new Course(NAME, TITLE, SECTION, CREDITS, INSTRUCTOR_ID, MEETING_DAYS, START_TIME, END_TIME);
	...
	/**
	 * Test Activity.checkConflict() for adding conflicting courses
	 */
	@Test
	public void testAddCourseToScheduleWithConflict() {
		WolfScheduler ws = new WolfScheduler(validTestFile);
		
		//Activity a1 = new Course(CSC 230,C and Software Tools,001,3,dbsturgi,MW,1145,1300);
		assertTrue(ws.addCourseToSchedule("CSC 230", "001"));
		
	    //Activity a2 = new Course("CSC 217", "Software Development Fundamentals Lab", "211", 1, "sesmith5", "T", 830, 1020);
	    assertTrue(ws.addCourseToSchedule("CSC 217", "211"));
	    
	    //Activity a3 = new Course("CSC 116", "Intro to Programming - Java", "002", 3, "jtking", "MW", 1120, 1310);	    
	 ...
	 */
	@Test
	public void testAddEventToScheduleWithConflict() {
		WolfScheduler ws = new WolfScheduler(validTestFile);

		ws.addEventToSchedule(EVENT_TITLE, EVENT_MEETING_DAYS, EVENT_START_TIME, EVENT_END_TIME, EVENT_DETAILS);
		
		//before endTime of exercise		
		try {
			ws.addEventToSchedule("Movie-time", "M", 830, 1000, EVENT_DETAILS);
  			fail();
  		} catch (IllegalArgumentException e) {
  			assertEquals("The event cannot be added due to a conflict.", e.getMessage());
  		}
		
		//same start time as exercise		
		try {
			ws.addEventToSchedule("Breakfast", "M", 800, 830, EVENT_DETAILS);
  			fail();
  		} catch (IllegalArgumentException e) {
  			assertEquals("The event cannot be added due to a conflict.", e.getMessage());
  		}
		
		//starts at exercise end time		
		try {
			ws.addEventToSchedule("Sleep", "M", 900, 1000, EVENT_DETAILS);
  			fail();
  		} catch (IllegalArgumentException e) {
  			assertEquals("The event cannot be added due to a conflict.", e.getMessage());
  		}
